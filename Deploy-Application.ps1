...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Copy-item -Path "$dirSupportFiles\license.dat" -Destination "$envProgramFiles\MATLAB\R2019a\license.dat"
		...
		Remove-File -Path "$envProgramData\Microsoft\Windows\Start Menu\Programs\MATLAB R2019a\Activate MATLAB R2019a.lnk"
		Remove-File -Path "$envProgramData\Microsoft\Windows\Start Menu\Programs\MATLAB R2019a\Deactivate MATLAB R2019a.lnk"
		Execute-Process -Path "$envSystem32Directory\netsh.exe" -Parameters "advfirewall firewall add rule name=`"MATLAB R2019a`" dir=in action=allow program=`"C:\program files\matlab\r2019a\bin\win64\matlab.exe`" description=`"MATLAB R2019a`" enable=yes profile=any protocol=tcp edge=deferuser" -WindowStyle "Hidden"
		Execute-Process -Path "$envSystem32Directory\netsh.exe" -Parameters "advfirewall firewall add rule name=`"MATLAB R2019a`" dir=in action=allow program=`"C:\program files\matlab\r2019a\bin\win64\matlab.exe`" description=`"MATLAB R2019a`" enable=yes profile=any protocol=udp edge=deferuser" -WindowStyle "Hidden"
